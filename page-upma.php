...
<?php 
    ...
    if (isset($_POST['upload'])) {
        $cantidad_borrados = borrar_mac_previos();
    ...
    if (isset($_POST['upload'])) {
        ...
        if ( file_exists($uploadfile) ) { // si un archivo con el mismo nombre ya existe se añade un sufijo al nombre
                ...
                $uploadfile = $uploaddir.'/'.$fecha_nombre.'1-'.$count.'.'.$exten;
                }
        } // fin if file_exists
    ...
    function iterador_csv($csv){
        ...
        for ( $x = 1 ;$x <= sizeof($csv); $x++){
            ...
            crear_mac($titulo, $cat, $mac, $pantalla, $capacidad, $ram, $precio);
        ...
        function crear_mac($titulo, $cat , $mac , $pantalla , $capacidad , $ram, $precio){
